...
def Conv_2D(x, output_chan, kernel=[5,5], stride=[2,2],padding="SAME" ,activation=tf.nn.relu, use_bn=False, train_phase=True,add_summary=False,name="Conv_2D",q1=0,q2=0):
	...
	with tf.variable_scope(name) as scope:
		...
		try:
			tf.add_conv_layer(filter=[q1,q2],strides=strd,padding=padding)
			tf.add_conv_layer(filter=[q1,q2],strides=strd,padding=padding)
		except:
			pass
